...
为控制复杂度，当前版本优先支持 Flat、IVF 和 HNSW 三类索引。其中，Flat 用作精确检索基线，IVF 和 HNSW 用于体现近似检索的效率优势。查询距离优先支持 L2 与 Cosine 两种方式。
...
普通用户主要使用前台检索功能；管理员主要负责账户和系统维护。两类流程在登录后分流，避免后台管理功能与检索业务混在一起。
...
普通用户注册后可以登录进入前台界面，管理员登录后进入后台界面。若账号或密码错误，系统返回明确提示并允许重新登录。
...
该流程主要解决检索前的数据准备问题。当前版本中，系统优先支持 CSV 或 NPY 格式的数据输入。用户可以上传新数据，也可以选择已有数据集。

系统读取数据后，首先检查样本数、特征维度、文件格式和缺失情况。数据通过校验后，再进行基础规范化和向量组织，输出可直接用于建索引和查询的细胞向量集合及其元信息。
...
为控制实现复杂度，当前版本优先考虑 Flat、IVF 和 HNSW 三类索引。其中 Flat 用作精确检索基线，IVF 和 HNSW 用于体现近似检索的效率优势。系统支持索引构建、保存、加载、切换和状态查看。
...
系统根据当前数据集和索引状态执行精确检索或 ANN 检索，返回排序后的 top-k 结果，同时展示距离值、查询耗时和所用索引类型。当前版本以单次查询为主，不涉及复杂查询历史分析和多用户并发检索场景。
...
该流程主要包括用户信息查看、基础账户维护和日志查看。管理员登录后台后，可以进行新增、修改、删除、查询等基础操作。
...
在系统内部，最核心的数据流为“数据导入 → 预处理 → 向量组织 → 索引构建/加载 → 查询执行 → 结果展示与评测”。其中，向量库和索引库是查询过程中的关键数据支撑，结果与评测库用于保存检索输出和评测记录，日志库则贯穿整个运行过程。

最终提交时，数据流图建议按更标准的 DFD 画法重新绘制，避免保留明显的流程图风格。

![主要处理层数据流图](figs/main-processing-layer-data-flow.png)

### 数据字典

在本次实现中，考虑到单细胞向量和索引文件体积较大，系统采用“元信息入库、数据文件落盘”的方式进行管理。数据库主要存储对象标识、路径、状态和参数信息，向量本体和索引文件存储在文件系统中。
...
| --- | --- | --- | --- | --- | --- |
| D1 | 用户信息库 | 存储用户账号、密码、角色、状态等信息 | user_id, username, password_hash, role, status, created_at | 注册、管理员维护 | 用户认证、后台管理 |
| D2 | 数据集库 | 存储单细胞数据集及其元信息 | dataset_id, dataset_name, source_type, file_path, sample_count, feature_count, upload_time, dataset_status | 数据导入模块 | 预处理、查询关联 |
| D3 | 向量集库 | 存储向量集元信息和存储位置 | vector_set_id, dataset_id, vector_dim, vector_file_path, preprocess_method, vector_status | 向量组织模块 | 索引构建、查询检索 |
| D4 | 索引库 | 存储索引描述信息 | index_id, dataset_id, index_type, index_param, build_time, index_path, index_status | 索引构建模块 | 查询检索、索引管理 |
| D5 | 结果与评测库 | 存储查询记录、结果记录和评测记录 | result_id, query_id, index_id, top_k, query_mode, query_time_ms, metric_name, metric_value, eval_time | 检索模块、评测模块 | 结果展示、性能分析 |
| D6 | 系统日志库 | 存储运行日志、异常信息和操作记录 | log_id, user_id, operation_type, operation_time, log_level, message | 各处理模块 | 系统维护、问题追踪 |
...
| 数据上传信息 | 用户提交的数据内容 | 数据文件、数据集名称、备注信息 | 普通用户 | 数据导入与预处理 |
| 数据校验结果 | 数据有效性检查结果 | 是否有效、错误原因、处理状态 | 数据导入与预处理 | 普通用户 |
| 规范化数据 | 经基础处理后的结构化数据 | 规范化矩阵、元信息、预处理方式 | 数据导入与预处理 | 向量组织与管理 |
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| dataset_id | 数据集编号 | String / Int | 数据集唯一标识 |
| dataset_name | 数据集名称 | String | 由用户命名或系统生成 |
| source_type | 数据来源类型 | String | 上传数据 / 系统已有数据 |
| file_path | 文件路径 | String | 数据文件存储位置 |
| sample_count | 样本数量 | Int | 细胞数量 |
| feature_count | 特征数量 | Int | 特征维度 |
| upload_time | 上传时间 | Datetime | 数据导入时间 |
| dataset_status | 数据状态 | String | 待处理、可用、异常等 |
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| vector_set_id | 向量集编号 | String / Int | 向量数据集唯一标识 |
| dataset_id | 所属数据集编号 | String / Int | 对应原始数据集 |
| vector_dim | 向量维度 | Int | 向量长度 |
| vector_file_path | 向量文件路径 | String | 向量文件保存位置 |
| preprocess_method | 预处理方式 | String | 记录预处理方法 |
| vector_status | 向量状态 | String | 可检索、失效等 |

当前版本在数据库层面仅记录向量集元信息和存储路径，具体向量内容采用文件方式保存。
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| index_id | 索引编号 | String / Int | 索引唯一标识 |
| dataset_id | 关联数据集编号 | String / Int | 对应的数据集 |
| index_type | 索引类型 | String | Flat、IVF、HNSW |
| index_param | 索引参数 | String / JSON | 索引构建参数配置 |
| build_time | 构建时间 | Datetime | 索引生成时间 |
| index_path | 索引路径 | String | 索引文件保存位置 |
| index_status | 索引状态 | String | 已构建、已加载、不可用等 |
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| query_id | 查询编号 | String / Int | 单次检索任务标识 |
| user_id | 发起用户编号 | String / Int | 检索请求发起者 |
| dataset_id | 查询数据集编号 | String / Int | 本次查询的数据集 |
| query_mode | 查询方式 | String | 精确检索 / ANN 检索 |
| query_object | 查询对象 | String / Array | 细胞编号或查询向量 |
| distance_metric | 距离度量方式 | String | L2 / Cosine |
| top_k | 返回数量 | Int | 返回前 k 个结果 |
| query_time | 查询发起时间 | Datetime | 请求提交时间 |
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| result_id | 结果编号 | String / Int | 结果记录唯一标识 |
| query_id | 对应查询编号 | String / Int | 关联查询请求 |
| rank_no | 排名序号 | Int | 结果在返回列表中的位置 |
| result_cell_id | 返回细胞编号 | String / Int | 检索到的相似细胞 |
| distance_value | 距离值 | Float | 用于衡量相似性 |
| query_time_ms | 查询耗时 | Float | 单次检索耗时 |
| index_id | 所用索引编号 | String / Int | 本次查询使用的索引 |
| index_type_snapshot | 索引类型快照 | String | 本次查询使用的索引类型 |
...
| 字段名 | 含义 | 类型 | 说明 |
| --- | --- | --- | --- |
| eval_id | 评测编号 | String / Int | 评测任务唯一标识 |
| dataset_id | 评测数据集编号 | String / Int | 参与评测的数据集 |
| index_id | 被评测索引编号 | String / Int | 参与对比的索引 |
| metric_name | 指标名称 | String | Recall@k、平均查询时间等 |
| metric_value | 指标值 | Float | 具体评测结果 |
| eval_time | 评测时间 | Datetime | 评测执行时间 |
| eval_note | 评测说明 | String | 参数设置或实验说明 |
...
该模块负责用户注册、登录、退出和角色识别。普通用户登录后进入检索界面，管理员登录后进入后台维护界面。考虑到本项目重点在检索功能，权限管理仅实现基础账号控制和用户维护，不扩展复杂权限分级。
...
模块输入主要是数据文件或已有数据集标识；主要处理包括读取、校验、规范化和向量组织；主要输出是数据集元信息与向量集存储信息。这里重点解决的是让数据能够稳定进入检索主链路，而不是复现完整的单细胞分析流程。

#### 索引构建与管理模块

当前版本优先实现 Flat、IVF 和 HNSW 三类索引。Flat 用于精确检索，IVF 和 HNSW 用于近似检索对比。系统支持索引构建、保存、加载、切换和状态查看。
...
| ANN 结果可接受 | 默认参数下 Recall@10 不低于 0.8 |
...
| 相对效率体现明显 | ANN 查询平均耗时应明显低于对应精确检索 |
...
本系统后端推荐采用 Python 3.9 及以上版本实现，Web 服务框架采用 Flask，数据处理依赖 NumPy、Pandas 和 scikit-learn，向量检索依赖 Faiss。前端通过主流浏览器访问即可。若后续需要扩展，也可替换为其他兼容框架，但不属于本次实现重点。
...
| 项目 | 当前版本采用 | 说明 |
| --- | --- | --- |
| Python 版本 | Python 3.9+ | 作为主要后端运行环境 |
| Web 框架 | Flask | 实现接口服务和页面交互 |
| 数据处理库 | NumPy、Pandas | 用于矩阵读取和基础处理 |
| 预处理与评测辅助 | scikit-learn | 用于基础预处理和评测辅助 |
...
| 向量数据文件 | 本地文件系统 | 保存处理后的向量数据 |
| 索引文件 | 本地文件系统 | 保存索引本体 |
| 查询结果与评测记录 | SQLite | 便于结果追踪和性能比较 |
| 日志信息 | 日志文件 | 便于调试和问题定位 |
...
本需求分析报告围绕单细胞高维向量数据的 ANN 检索系统展开，当前版本重点明确了系统在课程作业中的实现范围、业务主线、核心数据、功能模块、质量要求和运行条件。整体设计以检索主链路为核心，通过数据导入、向量组织、索引构建、查询执行、结果展示和性能评测等环节，形成了一个结构清晰、边界明确、便于演示和继续细化的课程项目方案。

后续设计工作将继续围绕本报告中的边界和选择展开，重点完成图的进一步规范绘制、关系模式设计、数据库实现和系统功能细化。在课程作业范围内，本报告的作用不是把系统描述得过大、过全，而是为后续实现提供一个可以直接落地的分析基础。
