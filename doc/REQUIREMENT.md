...
7. 支持共享向量空间中的多数据集联合检索；
8. 支持证据可追溯的自然语言细胞检索与辅助分析。
...
当前版本重点实现数据导入、向量组织、索引构建、单数据集/联合查询、证据约束自然语言检索、结果展示和基础评测，不扩展到自动生物学注释、跨模态联合分析和大规模可视化探索。RAG 回答是检索辅助，不作为生物学定论或临床建议。
...
本系统的核心业务可以概括为：用户登录系统后导入单细胞数据，完成基础预处理和向量组织，再构建或加载索引，在设置查询参数后执行精确检索、ANN 检索、跨数据集检索或证据约束自然语言检索，并查看 top-k 结果、引用、查询耗时和评测信息；管理员则负责基础账户维护和运行支撑。
...
| 联合检索与 RAG | 共享空间确认、来源映射、自然语言计划、证据生成与引用校验 | 支持跨数据集搜索和可追溯辅助分析 |
...
联合检索只接受用户明确选择、维度一致且已确认处于同一向量空间的数据集。系统建立联合向量快照，并维护全局行号到数据集来源、本地细胞编号和名称的可逆映射。相同维度不是语义对齐证明，系统不自动对齐独立 PCA 或执行批次校正。

RAG 助手把自然语言解析为相似细胞、类型代表或数据概览三种受限计划。数据集范围、K、度量和共享空间仍由服务端校验；模型不能生成 SQL、路径或权限决策。检索证据使用 `[D#]`/`[E#]` 标识，生成输出必须通过引用白名单复验。无外部 provider 时返回本地证据摘要；原始向量、问题和回答不进入历史数据库。

#### 结果展示与性能评测模块

该模块负责展示 top-k 检索结果、距离值、查询耗时和索引类型等信息，并支持对不同索引方案进行基础性能对比。当前版本重点比较平均查询时间、Recall@k、索引构建时间和稳定的序列化索引字节数。
...
因此，当前版本的功能关系可以概括为：用户与权限管理提供入口，数据导入与预处理提供基础，索引管理提供支撑，单数据集/联合/RAG 查询构成核心，结果展示与评测负责反馈，系统运行支撑贯穿全程。
