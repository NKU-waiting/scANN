...
课程要求中的 Web 应用、用户模块、数据管理、索引构建/保存/加载/动态管理、细胞/向量 Top-K、条件检索、结果展示、性能评测均已实现。项目进一步完成了数据/索引指纹校验、多数据集联合检索、查询与评测历史、UMAP/PCA 高亮、登录门禁、生产配置保护和自动化质量门。
...
- 服务层：数据集事务、索引事务、线程安全单数据集/联合查询、评测、历史、投影缓存。
...
接口采用资源化分组：`auth`、`datasets`、`index`、`search`、`federated`、`eval`、`history`、`visualization`。完整请求/响应约束见 [API 文档](API.md)。

UI 为登录后的单页工作台，按“账户 → 数据 → 索引 → 联合检索 → 查询 → 评测 → 历史”组织。耗时操作有独立状态；影响活动资源的操作互斥；表格在窄屏可横向滚动；主要错误使用 alert 语义；UMAP/PCA 使用原生 SVG 并附图例和点标题。
